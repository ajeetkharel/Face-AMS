...
class AttendancePipeline:
    ...
    def __init__(self, class_based, _class, cam_idx):
        if class_based:
            self._init_db_(_class)
        self.cam_idx = cam_idx


        self.canvas = Canvas(_class)

    def _init_db_(self, _class):
    ...
    def check_routine(self, time, date):
        for index, row in self.routines.iterrows():
...
if __name__ == '__main__':
    ...
    ap.add_argument('--class', required=False, type=int,
                    help='In which class is the camera attached', default=0)
    ...
    pipeline = AttendancePipeline(class_based, _class_id, camera_idx)
